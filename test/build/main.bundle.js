...
var umbrella_min = createCommonjsModule(function (module) {
  ...
  var u = function u(t, e) {
    return this instanceof u
      ? t instanceof u
        ? t
        : ("string" == typeof t && (t = this.select(t, e)),
          t && t.nodeName && (t = [t]),
          void (this.nodes = this.slice(t)))
      : new u(t, e);
  };
  (u.prototype = {
    get length() {
      ...
      return (
        "number" == typeof t &&
          (t =
            0 === t
              ? []
              : new Array(t).join().split(",").map(Number.call, Number)),
    ...
    (u.prototype.filter = function (e) {
      var t = function t(_t) {
        return (
          (_t.matches =
            _t.matches || _t.msMatchesSelector || _t.webkitMatchesSelector),
          _t.matches(e || "*")
        );
      };
      return (
    ...
    (u.prototype.generate = function (t) {
      return /^\s*<tr[> ]/.test(t)
        ? u(document.createElement("table")).html(t).children().children().nodes
        : /^\s*<t(h|d)[> ]/.test(t)
        ? u(document.createElement("table"))
            ...
            .children().nodes
        : /^\s*</.test(t)
        ? u(document.createElement("div")).html(t).children().nodes
    ...
    (u.prototype.pairs = function (n, t, e, r) {
      ...
      return "object" == (typeof n === "undefined" ? "undefined" : _typeof2(n))
        ? this.each(function (t) {
            for (var e in n) {
              r(t, e, n[e]);
            }
    ...
    (u.prototype.serialize = function () {
      ...
      return this.slice(this.first().elements)
        .reduce(function (e, n) {
          return !n.name || n.disabled || "file" === n.type
            ? e
            : /(checkbox|radio)/.test(n.type) && !n.checked
            ? e
            : "select-multiple" === n.type
            ? (u(n.options).each(function (t) {
                t.selected && (e += "&" + r.uri(n.name) + "=" + r.uri(t.value));
              }),
              e)
            : e + "&" + r.uri(n.name) + "=" + r.uri(n.value);
        }, "")
        .slice(1);
    }),
    ...
    (u.prototype.slice = function (t) {
      return t &&
        0 !== t.length &&
        "string" != typeof t &&
        "[object Function]" !== t.toString()
        ? t.length
          ? [].slice.call(t.nodes || t)
          : [t]
        : [];
    }),
    ...
    (u.prototype.trigger = function (t) {
      ...
      return this.eacharg(t, function (t, e) {
        var n,
          r = { bubbles: !0, cancelable: !0, detail: o };
        try {
  ...
  createClass(Step, [
    {
      ...
      get: function get$$1() {
        ...
        if (!this.container) {
          var _image = umbrella_min(
            '<div role="figure" class="guided-tour-step-image">' +
              (this.image ? '<img src="' + this.image + '" />' : "") +
              "</div>"
          );
          var _title = umbrella_min(
            '<div role="heading" class="guided-tour-step-title">' +
              this.title +
              "</div>"
          );
          var content = umbrella_min(
            '<div class="guided-tour-step-content">' + this.content + "</div>"
          );
          var highlight = (this.highlight = umbrella_min(
            '<div class="guided-tour-step-highlight"></div>'
          ));
          highlight.on("click", this.context.action);
          var tooltip = (this.tooltip = umbrella_min(
            '<div role="tooltip" class="guided-tour-step-tooltip"></div>'
          ));
          var arrow = (this.arrow = umbrella_min(
            '<div aria-hidden="true" class="guided-tour-arrow"></div>'
          ));
          tooltip
            .append(arrow)
            .append(_image)
            .append(_title)
            .append(content)
          this.container = umbrella_min(
            '<div role="dialog" class="guided-tour-step' +
              (this.first ? " guided-tour-step-first" : "") +
              (this.last ? " guided-tour-step-last" : "") +
              '"></div>'
          );
      ...
      value: function position() {
        ...
        if (this.target && this.target.offsetParent !== null) {
          ...
          if (view.width / 2 > rect.left) {
            ...
            arrow.first().style.left =
              rect.width > 400 ? "18px" : clamp(rect.width / 2, 14, 386) + "px";
          } else {
            style.right = Math.max(view.width - rect.right, 40) + "px";
            arrow.first().style.right =
              view.width - rect.right < 40 || rect.width > 400
                ? "8px"
                : clamp(rect.width / 2 - 8, 14, 386) + "px";
          }
        } else {
...
var Tour = (function () {
  ...
  function Tour() {
    ...
    // console.log("in Tour function, arguments are:", arguments, "this", this);
    debugger;
    var options =
      arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : {};
    ...
    if (
      _typeof(this._options.steps) === "object" &&
  ...
  createClass(Tour, [
    ...
    },
    {
      ...
      value: function start() {
        ...
        // console.log("inside start function", arguments)

        var step =
          arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : 0;

        // console.log("step in start function", step);
        ...
        if (this._ready) {
          ...
          throw new Error(
            "Tour is not configured properly. Check documentation."
          );
        }
      },
    },
      ...
      value: function action(e) {
        if (this._active) {
          ...
          if (currentstep.actiontarget) {
            var actiontarget = umbrella_min(currentstep.target).find(
